...
namespace SGSV.Producto
{
    public partial class frmListarTiposProducto : MasterForm
    {
        ...
        private void mnuModificarTipoProducto_Click(object sender, EventArgs e)
        {
            try
            {
                var indice = dgvTiposProducto.SelectedCells[0].RowIndex;
                new frmEditarTipoProducto(Convert.ToInt32(dgvTiposProducto.Rows[indice].Cells[0].Value.ToString())).ShowDialog();
                Cargar();
            }
            catch (Exception)
            {
